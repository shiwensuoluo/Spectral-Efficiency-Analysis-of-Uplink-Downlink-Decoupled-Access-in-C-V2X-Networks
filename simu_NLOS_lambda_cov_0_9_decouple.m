...
%参数    计算额了频谱效率   覆盖概率
d =3;  % OBSERVATION WINDOW RADIUS  original——40
cov_tao=0.8;
n=61
for iiii = 1:4:n
    iiii
...
%                         sum(log2(1+sir_decouple(posti_case3)))/length(posti_case3);...
    ...
    cov_NLOS_decouple(1:4,j1)= [ (  length(find(sir_decouple(1,posti_case1)>=cov_tao))+...
                                                  length(find(sir_decouple(1,posti_case2)>=cov_tao)) )/total_v;...%宏基站 下行
                                                  length(find(sir_decouple(2,posti_case1)>=cov_tao)) /total_v;...%宏基站 行
                                              length(find(sir_decouple(1,posti_case4)>=cov_tao)) /total_v;...%小基站 下行
                                             ( length(find(sir_decouple(2,posti_case2)>=cov_tao))+...
                                                  length(find(sir_decouple(2,posti_case4)>=cov_tao)) )/total_v;   ]%小基站 上行
...
cov_pr_0_9_NLOS_decouple(1:4,iiii)=mean(cov_NLOS_decouple,2)
...
figure 
plot(0.1:0.4:6.1,cov_pr_0_9_NLOS_decouple(:,1:4:61));
legend('md','mu','sd','su');
